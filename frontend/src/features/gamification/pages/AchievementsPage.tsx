        ...
        <Col xs={24} md={12}>
          <Card>
            <div className="flex items-baseline justify-between">
              <Statistic title="Cấp độ" value={overview.level} />
              <Text className="text-ink-soft text-sm">{overview.totalXp} XP</Text>
            </div>
        ...
        <Col xs={24} md={12}>
          <Card>
            <Statistic
              ...
              suffix="ngày"
              prefix={<FireOutlined className={overview.streakConHomNay ? 'text-star' : 'text-ink-soft'} />}
            />
...
function HuyHieu({ badge }: { badge: BadgeItem }) {
  ...
  return (
    ...
    >
      <div
        className={`border-line flex w-40 flex-col items-center gap-1 rounded border px-3 py-3 text-center ${
          daMo ? 'bg-surface-subtle' : 'opacity-50'
        }`}
      >
        <span className={`text-2xl ${daMo ? '' : 'grayscale'}`} aria-hidden>
          {badge.icon ?? '🏅'}
        </span>
